...
const YandexMusicLogo = ({ size = 16, className, style }) => (
  <img src="/yandex_logo.png" alt="Yandex" width={size} height={size} className={className} style={{ display: "inline-block", verticalAlign: "middle", objectFit: "contain", ...style }} />
);

const SoundCloudLogo = ({ size = 16, className, style }) => (
  <img src="/soundcloud_logo.png" alt="SoundCloud" width={size} height={size} className={className} style={{ display: "inline-block", verticalAlign: "middle", objectFit: "contain", ...style }} />
);

const YouTubeLogo = ({ size = 16, className, style }) => (
  <img src="/youtube_logo.png" alt="YouTube" width={size} height={size} className={className} style={{ display: "inline-block", verticalAlign: "middle", objectFit: "contain", ...style }} />
);

const VkLogo = ({ size = 16, className, style }) => (
  <img src="/vk_logo.png" alt="VK" width={size} height={size} className={className} style={{ display: "inline-block", verticalAlign: "middle", objectFit: "contain", ...style }} />
);
const VkMusicLogo = ({ height = 18, className, style }) => (
  <img src="/vk_music_logo.png" alt="VK Музыка" height={height} className={className} style={{ display: "inline-block", verticalAlign: "middle", objectFit: "contain", ...style }} />
);
    ...
    >
      <div style={{ position: "relative", borderRadius: 12, overflow: "hidden", aspectRatio: "1/1" }}>
        ...
        <span style={{
          ...
          {track.source === "yandex" ? <YandexMusicLogo size={12}/> :
           track.source === "soundcloud" ? <SoundCloudLogo size={12}/> :
           track.source === "youtube" ? <YouTubeLogo size={12}/> :
           track.source === "vk" ? <VkLogo size={12}/> :
...
function App() {
  ...
  const [vkRecommendations, setVkRecommendations] = useState([]);
  const [vkUserTracks, setVkUserTracks] = useState([]);
  const [isLoadingVkHome, setIsLoadingVkHome] = useState(false);
  const [selectedVkMood, setSelectedVkMood] = useState("all");
  const [isChartExpanded, setIsChartExpanded] = useState(false);

  const vkMoods = [
    { id: "all", label: "Все треки", emoji: "✨", keywords: [] },
    { id: "energetic", label: "Бодрое", emoji: "⚡", keywords: ["remix", "dance", "club", "bass", "хит", "энергия"] },
    { id: "relax", label: "Расслабленное", emoji: "🌙", keywords: ["chill", "lofi", "lounge", "relax", "тишина", "slow"] },
    { id: "drive", label: "В дороге", emoji: "🚗", keywords: ["phonk", "drive", "авто", "машина", "ночь", "night"] },
    { id: "workout", label: "Тренировка", emoji: "🏋️", keywords: ["workout", "rock", "phonk", "gym", "кач", "hard"] },
    { id: "romantic", label: "Романтичное", emoji: "💖", keywords: ["love", "романтика", "любовь", "лирика", "pop", "душа"] },
    { id: "work", label: "Для работы", emoji: "💻", keywords: ["focus", "study", "lofi", "ambient", "piano"] },
    { id: "party", label: "Вечеринка", emoji: "🔥", keywords: ["party", "клуб", "тусовка", "dance", "mashup"] },
    { id: "melancholy", label: "Меланхолия", emoji: "🌧️", keywords: ["sad", "грусть", "лирика", "дождь", "панельки", "дрифт"] },
  ];

  const getFilteredVkRecs = () => {
    if (selectedVkMood === "all" || !vkRecommendations || vkRecommendations.length === 0) {
      return vkRecommendations;
    }
    const currentMoodObj = vkMoods.find(m => m.id === selectedVkMood);
    if (!currentMoodObj || !currentMoodObj.keywords || currentMoodObj.keywords.length === 0) {
      return vkRecommendations;
    }
    const kw = currentMoodObj.keywords.map(k => k.toLowerCase());
    const matches = vkRecommendations.filter(t => {
  ...
  const [isVkMixActive, setIsVkMixActive] = useState(false);
  ...
  const [lyricsFontSize, setLyricsFontSize] = useState(localStorage.getItem("lyrics_font_size") || "xl");
  const [lyricsTextStyle, setLyricsTextStyle] = useState(localStorage.getItem("lyrics_text_style") || "neon");
  ...
  const [isClosingNowPlaying, setIsClosingNowPlaying] = useState(false);
  ...
  const [followedArtists, setFollowedArtists] = useState(() => {
    try {
      const raw = JSON.parse(localStorage.getItem("followed_artists") || "[]");
  ...
  const isFollowingArtist = (artistId) => {
    ...
    return followedArtists.some(a => String(a.id) === String(artistId));
  };
  ...
  const [playlists, setPlaylists] = useState(() => {
    try {
      ...
      const UNWANTED = ["chill & deep", "lofi beats", "soundcloud", "top 50", "synthwave"];
      const validated = {};
      for (const [key, val] of Object.entries(raw)) {
        const lower = key.toLowerCase();
        if (UNWANTED.some(u => lower.includes(u))) continue;
  ...
  const handleArtistClick = (e, track) => {
    ...
    let artistsList = track.artists;
    if ((!artistsList || artistsList.length <= 1) && track.artist) {
      const names = track.artist.split(/, | & | feat\.? | ft\.? /i).map(s => s.trim()).filter(Boolean);
      if (names.length > 1) {
        artistsList = names.map(name => ({ id: name, name }));
      }
    }

    if (artistsList && artistsList.length > 1) {
      setArtistSelectTrack({ ...track, artists: artistsList });
    } else {
      const aId = track.artistId || track.artist_id || (track.artists && track.artists[0] && track.artists[0].id) || track.artist;
  ...
  const closeNowPlayingWithAnim = () => {
    ...
    setTimeout(() => {
      ...
      setIsClosingNowPlaying(false);
    }, 240);
  };

  const applySpeedPreset = (speedVal, isSlowedReverb = false) => {
    setPlaybackSpeed(speedVal);
    if (audio1Ref.current) audio1Ref.current.playbackRate = speedVal;
    if (audio2Ref.current) audio2Ref.current.playbackRate = speedVal;
    if (isSlowedReverb) {
      applyEqPreset("BassBoost", [6, 4, 0, -2, -4]);
      setUseEqualizer(true);
      localStorage.setItem("use_equalizer", "true");
      showToast("✨ Slowed & Reverb активирован", "info");
    }
  };
  ...
  const loadVkHomeData = async () => {
    const vkTok = localStorage.getItem("vk_token") || vkToken || "";
    setIsLoadingVkHome(true);
    try {
      const recs = await invoke("get_vk_recommendations", { token: vkTok, count: 30 }).catch(e => { console.log("VK Recs error:", e); return []; });
      setVkRecommendations(recs || []);
      if (vkTok) {
        const userTrks = await invoke("get_vk_user_tracks", { token: vkTok, count: 50 }).catch(e => { console.log("VK User tracks error:", e); return []; });
        setVkUserTracks(userTrks || []);
      }
    } catch (e) {
      console.log("Error loading VK Home data:", e);
    } finally {
      setIsLoadingVkHome(false);
    }
  };
...
useEffect(() => {
  ...
  // Listen to automatic token capture events (VK & Yandex)
  useEffect(() => {
    let unlistenVk;
    let unlistenYandex;
    const setupListeners = async () => {
      unlistenVk = await listen("vk_token_captured", (event) => {
        if (event.payload) {
          setVkToken(event.payload);
          localStorage.setItem("vk_token", event.payload);
          showToast("✓ Авторизация через Марусю (VK) успешно выполнена!", "success");
        }
      });
      unlistenYandex = await listen("yandex_token_captured", (event) => {
        const token = event.payload;
        if (token) {
          setYandexToken(token);
          localStorage.setItem("yandex_token", token);
          showToast("✓ Яндекс Токен успешно получен!", "success");
        }
      });
    };
    setupListeners();
    return () => {
      if (unlistenVk) unlistenVk();
      if (unlistenYandex) unlistenYandex();
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const handleVkTokenInput = (val) => {
    let clean = val.trim();
    if (clean.includes("access_token=")) {
      const parts = clean.split("access_token=");
      if (parts[1]) {
        clean = parts[1].split("&")[0];
        showToast("✓ Токен извлечен из ссылки!", "success");
      }
    }
    setVkToken(clean);
    localStorage.setItem("vk_token", clean);
  };
...
const getCleanYandexToken = () => {
  ...
  const toggleLike = async (track) => { if (!track) return;
    ...
    if (track.source === "vk" && !wasLiked) {
      const vkTok = localStorage.getItem("vk_token") || vkToken || "";
      if (vkTok && track.id.startsWith("vk_")) {
        const parts = track.id.split("_");
        if (parts.length >= 3) {
          const ownerId = parseInt(parts[1], 10);
          const audioId = parseInt(parts[2], 10);
          if (!isNaN(ownerId) && !isNaN(audioId)) {
            invoke("vk_add_audio", { token: vkTok, ownerId, audioId })
              .then(() => showToast(`❤️ Добавлено в VK Аудиозаписи`, "success"))
              .catch(e => console.log("VK add audio error", e));
          }
        }
      }
    }
  ...
  const initEqualizer = () => {
    ...
    try {
      ...
      if (!source1NodeRef.current && audio1Ref.current) {
        try { source1NodeRef.current = ctx.createMediaElementSource(audio1Ref.current); } catch(e){}
      }
      if (!source2NodeRef.current && audio2Ref.current) {
        try { source2NodeRef.current = ctx.createMediaElementSource(audio2Ref.current); } catch(e){}
      }

      if (!filtersRef.current.length && source1NodeRef.current) {
        ...
        try {
          source1NodeRef.current.connect(filters[0]);
          if (source2NodeRef.current) source2NodeRef.current.connect(filters[0]);
      ...
      console.log("Web Audio init fine", e);
    }
  };
  ...
  const preloadNextTrack = async (currentIndex, currentQueue) => {
    if (isMyWaveActive) return; // Completely disable preloading for My Wave
    let nextIndex = currentIndex + 1;
    if (isShuffle && !isMyWaveActive && currentQueue.length > 1) {
    ...
    try {
      ...
      else if (nextTrack.source === "vk") {
        let url = nextTrack.streamUrl || "";
        if (!url || !url.startsWith("http")) {
          try {
            const vkTok = localStorage.getItem("vk_token") || vkToken || "";
            url = await invoke("get_vk_stream", { trackId: nextTrack.id, title: nextTrack.title || "", artist: nextTrack.artist || "", token: vkTok });
          } catch(e) {}
        }
        streamUrl = url;
      }
      ...
      if (streamUrl) {
        preloadedTrackRef.current = { track: nextTrack, streamUrl };
      }
    } catch (err) {
  ...
  const currentPlayRequestIdRef = useRef(0);

  const playTrack = async (track, indexInQueue = -1, forceNoPreloadTransition = false, newQueue = null) => {
    if (!track || !track.id) return;
    const requestId = ++currentPlayRequestIdRef.current;
    ...
    try {
      ...
      if (requestId !== currentPlayRequestIdRef.current) return;
      ...
      if (!streamUrl) {
        if (!forceNoPreloadTransition && preloadedTrackRef.current && preloadedTrackRef.current.track && preloadedTrackRef.current.track.id === track.id && preloadedTrackRef.current.track.source === track.source) {
          wasPreloaded = true;
          streamUrl = preloadedTrackRef.current.streamUrl;
        }
        ...
        if (!wasPreloaded) {
          if (track.source === "soundcloud") streamUrl = await invoke("get_soundcloud_stream", { trackId: track.id });
            ...
            if (!url || !url.startsWith("http")) {
              try {
                const vkTok = localStorage.getItem("vk_token") || vkToken || "";
                url = await invoke("get_vk_stream", { trackId: track.id, title: track.title || "", artist: track.artist || "", token: vkTok });
              } catch(e) { console.log("Stream error:", e); }
            }
            streamUrl = url;
          }
          else if (track.source === "rzt") {
            let url = "";
            const token = getCleanYandexToken();
            const primaryArtist = (track.artist || "").split(/, | & | feat\.? | ft\.? /i)[0].trim();
            // 1. Direct Yandex Music track ID if extracted from RZT metadata
            if (track.streamUrl) {
              try {
                url = await invoke("get_yandex_stream", { trackId: track.streamUrl, token });
              } catch (e) { console.log("RZT Yandex direct stream error:", e); }
            }
            // 2. Search Yandex Music by artist & title
            if (!url) {
              try {
                const searchRes = await invoke("search_yandex", { query: `${primaryArtist} ${track.title}`, token });
                if (searchRes && searchRes.length > 0) {
                  url = await invoke("get_yandex_stream", { trackId: searchRes[0].id, token });
                }
              } catch (e) { console.log("RZT Yandex search stream error:", e); }
            }
            // 3. Fallback to SoundCloud / VK
            if (!url) {
              try {
                const scQuery = `${primaryArtist} ${track.title}`;
                const scTracks = await invoke("search_soundcloud", { query: scQuery });
                if (scTracks && scTracks.length > 0) {
                  url = await invoke("get_soundcloud_stream", { trackId: scTracks[0].id });
                }
              } catch (e) {}
            }
            if (!url) {
            ...
            let url = "";
            try {
              const scQuery = `${track.artist} ${track.title}`;
              const scTracks = await invoke("search_soundcloud", { query: scQuery });
              if (scTracks && scTracks.length > 0) {
                url = await invoke("get_soundcloud_stream", { trackId: scTracks[0].id });
              }
            } catch (e) {
              console.log("SoundCloud stream error:", e);
            }
            if (!url) {
              try {
                const vkTok = localStorage.getItem("vk_token") || vkToken || "";
                url = await invoke("get_vk_stream", { trackId: track.id, title: track.title || "", artist: track.artist || "", token: vkTok });
              } catch (e) { console.log("Stream fallback error:", e); }
            }
            streamUrl = url;
          }
        }
      ...
      if (!cachedPath && isLiked(track)) {
        invoke("cache_liked_track", { trackId: track.id, source: track.source, streamUrl })
          .then(() => showToast(`🎵 ${track.title}`, "success"))
          .catch(e => console.log("Failed to cache liked track", e));
      }

      let nq = newQueue ? [...newQueue] : [...queue];
      if (newQueue) {
        setQueue(nq);
      }
        ...
        await activeAudio.play().catch(async (err) => {
          if (err.name !== "AbortError") {
            ...
            if (track.source === "vk") {
              try {
                const vkTok = localStorage.getItem("vk_token") || vkToken || "";
                const fbUrl = await invoke("get_vk_stream", { trackId: track.id, title: track.title || "", artist: track.artist || "", token: vkTok });
                if (fbUrl && fbUrl !== streamUrl) {
                  activeAudio.src = fbUrl;
                  activeAudio.load();
  ...
  const [artistPlatformStats, setArtistPlatformStats] = useState(null);
  ...
  const openArtist = async (artistId, artistName) => {
    ...
    let rawName = artistName || artistId || "";
    if (typeof rawName !== "string") rawName = String(rawName);
    let cleanName = rawName.trim();
    
    // Strip raw numeric IDs or garbage placeholders
    if (!cleanName || /^\d+$/.test(cleanName) || cleanName === "Unknown") {
      cleanName = "Исполнитель";
    } else {
      // Strip feat/ft tags for cleaner search
      cleanName = cleanName.split(/ feat\.? | ft\.? | , | & /i)[0].trim();
    }

    setActiveArtist({ id: cleanName, name: cleanName, source: "official" });
    setActiveView("artist");
    setIsLoadingArtist(true);
    setActiveArtistBrief(null);
    setArtistPlatformStats(null);
    setErrorMessage("");
    ...
    try {
      ...
      if (cleanName !== "Исполнитель") {
        brief = await invoke("get_yandex_artist_brief", { artistId: cleanName, token }).catch(() => null);
      }

      if (brief && brief.artist && brief.artist.name) {
        setActiveArtistBrief(brief);
        invoke("get_artist_platform_stats", { artistName: brief.artist.name })
          .then(st => setArtistPlatformStats(st))
          .catch(e => console.log("Platform stats error:", e));
      } else if (cleanName !== "Исполнитель") {
        // Fallback to SoundCloud search for official artist profile
        const scTracks = await invoke("search_soundcloud", { query: cleanName }).catch(() => []);
        if (scTracks && scTracks.length > 0) {
          const firstSc = scTracks[0];
          setActiveArtistBrief({
            artist: {
              id: firstSc.artist,
              name: firstSc.artist,
              thumbnail: firstSc.thumbnail || "/tucus_logo.png",
              genres: ["Music"]
            },
            tracks: scTracks,
            albums: []
          });
          invoke("get_artist_platform_stats", { artistName: firstSc.artist })
            .then(st => setArtistPlatformStats(st))
            .catch(e => console.log("Platform stats error:", e));
        }
      }
  ...
  const openVkArtist = async (artistId, artistName) => {
    let cleanTarget = artistName;
    if (!cleanTarget || /^\d+$/.test(cleanTarget) || cleanTarget === "Unknown") {
      if (artistId && !/^\d+$/.test(artistId)) cleanTarget = artistId;
    }
    openArtist(cleanTarget || artistName || artistId);
  };
...
const openSoundCloudArtist = async (userId, artistName) => {
  ...
  const handleSearch = async (e, overrideSource) => {
    ...
    try {
      ...
      if (activeSrc === "all" || activeSrc === "vk") {
        const vkToken = localStorage.getItem("vk_token") || "";
        promises.push(invoke("search_vk_music", { query: rawQuery, token: vkToken }).then(d => ({ source:"vk", data:d||[] })).catch(e => ({ source:"vk", error:e.toString() })));
      }
  ...
  const startVkMix = async () => {
    setIsTrackLoading(true);
    showToast("✨ Запуск VK Микса...", "info");
    const vkTok = localStorage.getItem("vk_token") || vkToken || "";
    try {
      const recs = await invoke("get_vk_recommendations", { token: vkTok, count: 50 });
      if (recs && recs.length > 0) {
        setVkRecommendations(recs);
        setIsVkMixActive(true);
        setIsMyWaveActive(false);
        playTrack(recs[0], 0, false, recs);
        showToast("✨ Персональный VK Микс запущен", "success");
      } else {
        showToast("Не удалось запустить VK Микс", "error");
      }
    } catch(e) {
      console.error("VK Mix start error:", e);
      showToast(e?.toString() || "Ошибка запуска VK Микса", "error");
    } finally {
  ...
  const handleNext = async () => {
    ...
    if (isVkMixActive && ni >= queue.length - 3) {
      const vkTok = localStorage.getItem("vk_token") || vkToken || "";
      invoke("get_vk_recommendations", { token: vkTok, count: 30 })
        ...
        }).catch(e => console.log("VK Mix auto-refill error:", e));
    }
    ...
    if (isShuffle && !isMyWaveActive && queue.length > 1) ni = Math.floor(Math.random() * queue.length);
    ...
    <div className="app-root" onClick={() => setSelectedTrackForPlaylist(null)}>
      ...
      <audio 
        ...
        onError={() => { 
          if (activePlayer === 1) { 
            if (currentTrack) {
              invoke("delete_cached_track", { trackId: currentTrack.id, source: currentTrack.source }).catch(()=>{});
            }
            setErrorMessage("⚠️ Не удалось загрузить трек, переключаем..."); 
            setIsPlaying(false);
      ...
      {showNowPlaying && currentTrack && (() => {
        ...
        return (
          <div className={`np-modal ${isClosingNowPlaying ? "np-modal--closing" : ""}`} onClick={closeNowPlayingWithAnim}>
            ...
            <div className={`np-modal__content ${showRightPanel ? "" : "np-modal__content--centered"}`} onClick={e => e.stopPropagation()}>
              <div className="np-modal__left">
                ...
                <div className="np-modal__meta">
                  ...
                  <div className="np-modal__artist" style={{ cursor: "pointer", textDecoration: "underline" }} onClick={(e) => { closeNowPlayingWithAnim(); handleArtistClick(e, currentTrack); }}>{currentTrack.artist}</div>
              ...
              <div className="np-modal__actions" style={{ position: "absolute", top: 24, right: 24, display: "flex", gap: 12 }}>
                ...
                <button className="np-modal__action-btn" onClick={() => { closeNowPlayingWithAnim(); setShowCinemaMode(true); }} title="Режим Кинотеатр"><Maximize2 size={18}/></button>
              </div>
            </div>
          </div>
        );
      ...
      <div className="layout" style={activeView === "wave" ? { gridTemplateColumns: "1fr" } : {}}>
        <aside className="sidebar glass-sidebar" style={activeView === "wave" ? { display: "none" } : {}}>
          ...
          <nav className="sidebar__nav" style={{ marginTop: 6 }}>
            ...
            {showVkHome && (
              <button className={`nav__item ${activeView==="vkhome"?"nav__item--active":""}`} onClick={()=>handleSidebarClick("vkhome")} title="VK Музыка">
                <VkMusicLogo height={16} />
              </button>
            )}
            ...
            <button className={`nav__item ${activeView==="following"?"nav__item--active":""}`} onClick={()=>handleSidebarClick("following")}>
              <Users size={16}/><span>Подписки</span>
              {followedArtists.length>0&&<span className="nav__count">{followedArtists.length}</span>}
            </button>
          ...
          <div className="sidebar__playlists">
            ...
            <div className="sidebar__playlist-list">
              {Object.keys(playlists).filter(name => !name.toLowerCase().includes("soundcloud")).map(name=>(
        ...
        <main className="main-content" style={activeView === "wave" ? { padding: 0, gap: 0 } : {}}>
          ...
          {activeView==="search"&&(
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              <div className="source-tabs">
                {["all","yandex","soundcloud","youtube","vk"].map(src=>(
                  <button key={src} className={`source-tab source-tab--${src} ${searchSource===src?"source-tab--active":""}`} onClick={() => { setSearchSource(src); if (searchQuery.trim()) handleSearch(null, src); }}>
                    {src==="all"?"Все":src==="yandex"?"Яндекс":src==="soundcloud"?"SoundCloud":src==="youtube"?"YouTube Music":src==="vk"?"VK Музыка":""}
                  </button>
                ))}
              </div>
          ...
          {activeView==="home"&&(
            <div className="home-page">
              ...
              {/* Новые релизы (Риса за Творчество & Multi-sources) */}
              {(homeRecommendations?.releases?.length > 0 || homeRecommendations?.rztReleases?.length > 0 || homeRecommendations?.recommendations?.length > 0) && (
                <div className="home-section" style={{ marginBottom: 36 }}>
                  <div className="home-section__header" style={{ marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <img 
                        src="/rzt_logo.svg" 
                        alt="Риса за Творчество" 
                        style={{ height: 20, width: "auto", objectFit: "contain", filter: "brightness(0.95)" }} 
                      />
                      <span style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)" }}>Новые релизы</span>
                    </div>
                    <span style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", fontWeight: 600 }}>risazatvorchestvo.com • Свежие новинки</span>
                  </div>
                  <div 
                  ...
                  >
                    {(homeRecommendations?.releases || homeRecommendations?.rztReleases || homeRecommendations?.recommendations || []).slice(0, 30).map((t, i) => {
                      const recList = homeRecommendations?.releases || homeRecommendations?.rztReleases || homeRecommendations?.recommendations || [];
                      return (
                        <div 
                          key={`homerec-${t.source}-${t.id}-${i}`}
                          className="vk-release-card"
                          ...
                          onClick={() => {
                            ...
                            const recList = homeRecommendations?.releases || homeRecommendations?.rztReleases || homeRecommendations?.recommendations || [];
                            const playableTracks = recList.filter(item => !item.explanation?.includes("is_album:1") && item.type !== "album");
                            const playableIndex = playableTracks.findIndex(pt => pt.id === t.id);
                            const playIdx = playableIndex >= 0 ? playableIndex : 0;
                            setQueue(playableTracks);
                            playTrack(t, playIdx, false, playableTracks);
                          }}
                        >
                          <div className="vk-release-card__cover-wrapper">
                            <img 
                              src={t.thumbnail || "/tucus_logo.png"} 
                              alt={t.title} 
                              className="vk-release-card__img" 
                              onError={(e) => { e.target.onerror = null; e.target.src = "/tucus_logo.png"; }}
                            />
                            <div className="vk-release-card__play-btn">
                              <Play size={22} fill="currentColor" style={{ marginLeft: 2 }} />
                            </div>
                            {t.source !== "rzt" && (
                              <div className="vk-release-card__badge" style={{ background: t.source === "vk" ? "#0077ff" : t.source === "lastfm" ? "#d51007" : "rgba(0,0,0,0.6)" }}>
                                {t.source === "vk" ? <VkLogo size={12} /> : t.source === "lastfm" ? <span style={{ fontSize: 9, fontWeight: 900, color: "#fff" }}>fm</span> : <Music size={12} />}
                              </div>
                            )}
                          </div>
                          <div className="vk-release-card__title" title={t.title}>{t.title}</div>
                          <div className="vk-release-card__artist" title={t.artist}>{t.artist}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              {/* Чарт (Expandable Large Cover Grid on Home) */}
              {chartTracks.length > 0 && (
                <div className="home-section" style={{ marginBottom: 36 }}>
                  <div className="home-section__header" style={{ marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <TrendingUp size={18} style={{ color: "#ff5500" }} />
                      <span style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)" }}>Чарт</span>
                    </div>
                    <button 
                    ...
                    >
                      {isChartExpanded ? "Свернуть" : `Показать полностью (${chartTracks.length})`}
                    </button>
                  </div>
                  <div className="vk-releases-grid">
                    {(isChartExpanded ? chartTracks : chartTracks.slice(0, 6)).map((t, i) => (
                      <div 
                        key={`homechart-${t.source}-${t.id}-${i}`}
                        className="vk-release-card"
                        onClick={() => {
                          const idx = chartTracks.findIndex(x => x.id === t.id && x.source === t.source);
                          setQueue(chartTracks);
                          playTrack(t, idx !== -1 ? idx : 0, false, chartTracks);
                        }}
                      >
                        <div className="vk-release-card__cover-wrapper">
                          <img 
              ...
              {/* Недавно прослушано (At the VERY BOTTOM of Home) */}
              {homeRecent.length > 0 && (
                <div className="home-section" style={{ marginBottom: 36 }}>
                  <div className="home-section__header" style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                    <Clock size={18}/>
                    <span style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)" }}>Недавно прослушано</span>
                  </div>
                  <div className="vk-releases-grid">
                    {homeRecent.slice(0, 10).map((t, i) => (
                      <div 
                        key={`homerecent-${t.source}-${t.id}-${i}`}
                        className="vk-release-card"
                        onClick={() => {
                          setQueue(homeRecent);
                          playTrack(t, i, false, homeRecent);
                        }}
                      >
                        <div className="vk-release-card__cover-wrapper">
                          <img 
          ...
          {activeView==="chart"&&!isLoading&&(
            <div>
              ...
              <div className="track-list">
                {chartTracks.map((t, i) => renderTrackRow(t, i, {
                  key: `fullchart-${t.source}-${t.id}-${i}`,
                  playTrack: (trk) => {
                    setQueue(chartTracks);
                    playTrack(trk, i);
                  }
          ...
          {activeView === "following" && (
            <div className="following-view">
              <div className="view-header" style={{ marginBottom: 28, display: "flex", alignItems: "center", gap: 20 }}>
                <div className="view-header__cover glass-panel" style={{ width: 80, height: 80, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, rgba(255,204,0,0.25), rgba(0,0,0,0.6))", flexShrink: 0 }}>
                  <Users size={36} color="#FFCC00" />
                </div>
                <div className="view-header__info">
                  <div className="view-header__type" style={{ fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,0.5)", letterSpacing: "1px" }}>LIBRARY</div>
                  <h1 className="view-header__title" style={{ fontSize: 28, fontWeight: 900, color: "#fff", margin: "4px 0" }}>Подписки</h1>
                  <div className="view-header__meta" style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>
                    {followedArtists.length} исполнителей
                  </div>
                </div>
              </div>
              ...
              {followedArtists.length === 0 ? (
                <div className="empty-state" style={{ padding: "60px 0", textAlign: "center" }}>
                  <Users size={48} className="empty-state__icon" style={{ opacity: 0.3, marginBottom: 12 }} />
                  <div className="empty-state__title" style={{ fontSize: 16, fontWeight: 700, color: "#fff" }}>Нет подписок</div>
                  <div className="empty-state__sub" style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 4 }}>Нажмите "+ Подписаться" на странице любого исполнителя</div>
                </div>
              ) : (
                <div className="following-sections" style={{ display: "flex", flexDirection: "column", gap: 32 }}>
                  {/* Yandex Music Artists */}
                  {followedArtists.filter(a => a.source === "yandex" || !a.source).length > 0 && (
                    <div className="home-section">
                      <div className="home-section__header" style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                        <img src="/yandex_logo.png" alt="" style={{ height: 16 }} />
                        <span style={{ fontSize: 16, fontWeight: 800, color: "var(--text-primary)" }}>Яндекс Музыка (Подписки)</span>
                      </div>
                      <div className="releases-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 16 }}>
                        {followedArtists.filter(a => a.source === "yandex" || !a.source).map(artist => (
                          <div key={`fol-${artist.id}`} className="artist-card glass-panel clickable" onClick={() => openArtist(artist.id, artist.name)} style={{ padding: 14, borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 10, cursor: "pointer" }}>
                            <img src={artist.thumbnail || "/tucus_logo.png"} alt={artist.name} style={{ width: 90, height: 90, borderRadius: "50%", objectFit: "cover" }} onError={e=>{ e.target.onerror=null; e.target.src="/tucus_logo.png"; }} />
                            <div style={{ fontWeight: 700, fontSize: 13.5, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", width: "100%" }}>{artist.name}</div>
                            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>Исполнитель</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* SoundCloud Artists */}
                  {followedArtists.filter(a => a.source === "soundcloud").length > 0 && (
                    <div className="home-section">
                      <div className="home-section__header" style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                        <img src="/soundcloud_logo.png" alt="" style={{ height: 16 }} />
                        <span style={{ fontSize: 16, fontWeight: 800, color: "var(--text-primary)" }}>SoundCloud (Подписки)</span>
                      </div>
                      <div className="releases-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 16 }}>
                        {followedArtists.filter(a => a.source === "soundcloud").map(artist => (
                          <div key={`fol-sc-${artist.id}`} className="artist-card glass-panel clickable" onClick={() => openArtist(artist.id, artist.name)} style={{ padding: 14, borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 10, cursor: "pointer" }}>
                            <img src={artist.thumbnail || "/tucus_logo.png"} alt={artist.name} style={{ width: 90, height: 90, borderRadius: "50%", objectFit: "cover" }} onError={e=>{ e.target.onerror=null; e.target.src="/tucus_logo.png"; }} />
                            <div style={{ fontWeight: 700, fontSize: 13.5, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", width: "100%" }}>{artist.name}</div>
                            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>SoundCloud Artist</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* Other / Custom Artists */}
                  {followedArtists.filter(a => a.source && a.source !== "yandex" && a.source !== "soundcloud").length > 0 && (
                    <div className="home-section">
                      <div className="home-section__header" style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                        <Users size={16} />
                        <span style={{ fontSize: 16, fontWeight: 800, color: "var(--text-primary)" }}>Другие платформы</span>
                      </div>
                      <div className="releases-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 16 }}>
                        {followedArtists.filter(a => a.source && a.source !== "yandex" && a.source !== "soundcloud").map(artist => (
                          <div key={`fol-oth-${artist.id}`} className="artist-card glass-panel clickable" onClick={() => openArtist(artist.id, artist.name)} style={{ padding: 14, borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 10, cursor: "pointer" }}>
                            <img src={artist.thumbnail || "/tucus_logo.png"} alt={artist.name} style={{ width: 90, height: 90, borderRadius: "50%", objectFit: "cover" }} onError={e=>{ e.target.onerror=null; e.target.src="/tucus_logo.png"; }} />
                            <div style={{ fontWeight: 700, fontSize: 13.5, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", width: "100%" }}>{artist.name}</div>
                            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>Исполнитель</div>
                          </div>
                      ...
                      <div className="view-header">
                        ...
                        <div className="artist-detail__info">
                          ...
                          <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>
                            {activeArtistBrief.tracks?.length || 0} треков
                          </div>

                          {/* Follow Artist Button */}
                          {activeArtistBrief?.artist && (
                            <button
                              className="follow-btn glass-panel"
                              onClick={() => toggleFollowArtist(activeArtistBrief.artist)}
                              style={{
                                display: "inline-flex",
                                alignItems: "center",
                                gap: 6,
                                ...
                                background: isFollowingArtist(activeArtistBrief.artist.id) ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.06)",
                                color: "#fff",
                                fontWeight: 700,
                                fontSize: 13,
                                cursor: "pointer",
                            ...
                            >
                              {isFollowingArtist(activeArtistBrief.artist.id) ? (
                          ...
                          {/* Glass tiles for platform monthly listener & follower stats */}
                          <div style={{ display: "flex", gap: 12, marginTop: 14, flexWrap: "wrap", alignItems: "center" }}>
                            {activeArtistBrief.artist.monthlyListeners > 0 && (
                              <div style={{
                                display: "flex",
                                alignItems: "center",
                                gap: 10,
                                ...
                                <img src="/yandex_logo.png" alt="Yandex Music" style={{ height: 16, objectFit: "contain" }} />
                                <div>
                                  <div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>
                                    {activeArtistBrief.artist.monthlyListeners >= 1000000 
                                      ? (activeArtistBrief.artist.monthlyListeners / 1000000).toFixed(1) + "M"
                                      : activeArtistBrief.artist.monthlyListeners >= 1000
                                      ? (activeArtistBrief.artist.monthlyListeners / 1000).toFixed(1) + " тыс."
                                      : activeArtistBrief.artist.monthlyListeners}
                                  </div>
                                  <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.6)" }}>слушателей в месяц</div>
                                </div>
                              </div>
                            )}

                            {artistPlatformStats?.spotifyFollowers > 0 && (
                              <div style={{
                                display: "flex",
                                alignItems: "center",
                                gap: 10,
                                ...
                                <img src="/spotify_logo.png" alt="Spotify" style={{ height: 16, objectFit: "contain" }} />
                                <div>
                                  <div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>
                                    {artistPlatformStats.spotifyFollowers >= 1000000 
                                      ? (artistPlatformStats.spotifyFollowers / 1000000).toFixed(1) + "M"
                                      : artistPlatformStats.spotifyFollowers >= 1000
                                      ? (artistPlatformStats.spotifyFollowers / 1000).toFixed(1) + " тыс."
                                      : artistPlatformStats.spotifyFollowers}
                                  </div>
                                  <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.6)" }}>подписчиков Spotify</div>
                                </div>
                              </div>
                            )}

                            {artistPlatformStats?.soundcloudFollowers > 0 && (
                              <div style={{
                                display: "flex",
                                alignItems: "center",
                                gap: 10,
                                ...
                                <img src="/soundcloud_logo.png" alt="SoundCloud" style={{ height: 16, objectFit: "contain" }} />
                                <div>
                                  <div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>
                                    {artistPlatformStats.soundcloudFollowers >= 1000000 
                                      ? (artistPlatformStats.soundcloudFollowers / 1000000).toFixed(1) + "M"
                                      : artistPlatformStats.soundcloudFollowers >= 1000
                                      ? (artistPlatformStats.soundcloudFollowers / 1000).toFixed(1) + " тыс."
                                      : artistPlatformStats.soundcloudFollowers}
                                  </div>
                                  <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.6)" }}>подписчиков SoundCloud</div>
                                </div>
                              </div>
                            )}
                          </div>
          ...
          {activeView==="liked"&&!isLoading&&(
            <div>
              <div className="view-header" style={{ gap: 24, padding: "20px 0", display: "flex", alignItems: "center" }}>
                <div style={{ width: 140, height: 140, borderRadius: 16, overflow: "hidden", position: "relative", flexShrink: 0, boxShadow: "0 16px 40px rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.1)" }}>
                  {likedTracks.length > 0 ? (
                    <div style={{ display: "grid", gridTemplateColumns: likedTracks.length >= 4 ? "1fr 1fr" : "1fr", gridTemplateRows: likedTracks.length >= 4 ? "1fr 1fr" : "1fr", width: "100%", height: "100%" }}>
                      {likedTracks.slice(0, likedTracks.length >= 4 ? 4 : 1).map((t, idx) => (
                        <img key={`liked-art-${t.id}-${idx}`} src={t.thumbnail} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e=>e.target.style.opacity="0"} />
                      ))}
                    </div>
                  ) : (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", height: "100%", background: "linear-gradient(135deg, #ec4899 0%, #8b5cf6 100%)" }}>
                      <Heart size={54} fill="#fff" color="#fff" />
                    </div>
                  )}
                </div>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "var(--accent)", marginBottom: 4 }}>Плейлист</div>
                  <div className="view-header__title" style={{ fontSize: 32, fontWeight: 800 }}>Любимые треки</div>
                  <div className="view-header__sub" style={{ fontSize: 13, opacity: 0.6, marginTop: 4 }}>{likedTracks.length} треков в вашей коллекции</div>
                  {likedTracks.length > 0 && (
                    <button className="play-all-btn" style={{ marginTop: 14 }} onClick={()=>{setQueue(likedTracks);playTrack(likedTracks[0],0);}}>
                      <Play size={14} fill="currentColor"/> Играть все
                    </button>
                  )}
                </div>
              </div>
              {likedTracks.length===0
                ?<div className="empty-state"><Heart size={40} className="empty-state__icon"/><div className="empty-state__title">Нет любимых треков</div><div className="empty-state__sub">Нажмите сердечко на любом треке</div></div>
          ...
          {activeView==="playlist"&&activePlaylistName&&!isLoading&&(
            <div>
              <div className="view-header" style={{ gap: 24, padding: "20px 0", display: "flex", alignItems: "center" }}>
                <div style={{ width: 140, height: 140, borderRadius: 16, overflow: "hidden", position: "relative", flexShrink: 0, boxShadow: "0 16px 40px rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.1)" }}>
                  {((playlists[activePlaylistName]||[])[0]?.thumbnail) ? (
                    <img src={(playlists[activePlaylistName]||[])[0].thumbnail} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  ) : (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", height: "100%", background: "linear-gradient(135deg, #6366f1 0%, #a855f7 100%)" }}>
                ...
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "var(--accent)", marginBottom: 4 }}>Плейлист</div>
                  <div className="view-header__title" style={{ fontSize: 32, fontWeight: 800 }}>{activePlaylistName}</div>
                  <div className="view-header__sub" style={{ fontSize: 13, opacity: 0.6, marginTop: 4 }}>{(playlists[activePlaylistName]||[]).length} треков</div>
                  <div className="view-header__btns" style={{ marginTop: 14 }}>
                    {(playlists[activePlaylistName]||[]).length>0&&<button className="play-all-btn" onClick={()=>{ try { playPlaylist(activePlaylistName); } catch(e) { console.error(e); } }}><Play size={14} fill="currentColor"/> Играть все</button>}
                    {activePlaylistName !== "Yandex Liked" && (
                      <button className="delete-btn" onClick={()=>deletePlaylist(activePlaylistName)}><Trash2 size={14}/> Удалить</button>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="vk-home-view" style={{ padding: "10px 0 40px 0" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <img src="/vk_music_logo.png" alt="VK Музыка" style={{ height: 28, objectFit: "contain" }} />
                </div>
                <button 
                  onClick={loadVkHomeData} 
                  disabled={isLoadingVkHome}
                  style={{
                    display: "inline-flex", alignItems: "center", gap: 6,
                    background: "rgba(255, 255, 255, 0.08)", border: "1px solid rgba(255, 255, 255, 0.12)",
                    color: "#fff", padding: "8px 16px", borderRadius: 20, fontSize: 13, fontWeight: 600,
                    cursor: "pointer", backdropFilter: "blur(10px)"
                  }}
                >
                  <Zap size={14} className={isLoadingVkHome ? "spin" : ""} /> {isLoadingVkHome ? "Загрузка..." : "Обновить рекомендации"}
                </button>
              </div>

              {!vkToken && (
                <div className="glass-panel" style={{ padding: "14px 20px", borderRadius: 16, marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(0, 119, 255, 0.12)", border: "1px solid rgba(0, 119, 255, 0.25)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <img src="/vk_logo.png" alt="VK" style={{ width: 28, height: 28, borderRadius: 8 }} />
                    <span style={{ fontSize: 13, color: "rgba(255, 255, 255, 0.9)", fontWeight: 500 }}>
                      Привяжите VK в настройках для синхронизации с вашей личной медиатекой ВКонтакте
                    </span>
                  </div>
                  <button 
                    onClick={() => { setActiveView("settings"); setSettingsSubTab("vk"); }}
                    style={{ background: "#0077ff", color: "#fff", border: "none", padding: "6px 14px", borderRadius: 10, fontWeight: 600, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}
                  >
                    Войти в VK
                  </button>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, marginBottom: 36 }}>
                <div 
                  ...
                  onClick={() => {
                    const filtered = getFilteredVkRecs();
                    if (filtered && filtered.length > 0) {
                      setVkRecommendations(filtered);
                      setIsVkMixActive(true);
                      setIsMyWaveActive(false);
                      playTrack(filtered[0], 0, false, filtered);
                      showToast(`✨ Персональный VK Микс запущен`, "success");
                    } else {
                      startVkMix();
                    }
                  ...
                  style={{
                    ...
                    background: "linear-gradient(135deg, rgba(0, 119, 255, 0.28), rgba(255, 0, 128, 0.2))",
                ...
                >
                  <div style={{ position: "relative", zIndex: 2 }}>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(0,0,0,0.35)", padding: "4px 10px", borderRadius: 12, fontSize: 11, fontWeight: 700, color: "#fff", marginBottom: 12 }}>
                      <Zap size={12} fill="currentColor" /> VK МИКС
                    </div>
                    <h3 style={{ fontSize: 22, fontWeight: 800, color: "#fff", marginBottom: 6 }}>Персональный Микс</h3>
                    <p style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", marginBottom: 18 }}>Бесконечный поток музыки на основе трендов VK</p>
                    <button style={{ background: "#fff", color: "#000", border: "none", padding: "10px 20px", borderRadius: 20, fontWeight: 700, fontSize: 13, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>
                      <Play size={14} fill="currentColor" /> Слушать VK Микс
                    </button>
                  </div>
                </div>
                ...
                <div 
                  className="glass-panel clickable" 
                  onClick={async () => {
                    let trks = vkUserTracks;
                    if (!trks || trks.length === 0) {
                      const vkTok = localStorage.getItem("vk_token") || vkToken || "";
                      if (!vkTok) {
                        showToast("Привяжите VK токен в настройках, чтобы слушать свои аудиозаписи", "info");
                        setActiveView("settings");
                        setSettingsSubTab("vk");
                        return;
                      }
                      showToast("⏳ Загрузка ваших аудио VK...", "info");
                      trks = await invoke("get_vk_user_tracks", { token: vkTok, count: 50 }).catch(() => []);
                      if (trks && trks.length > 0) setVkUserTracks(trks);
                    }
                    if (trks && trks.length > 0) {
                      playTrack(trks[0], 0, false, trks);
                    } else {
                      showToast("У вас пока нет сохраненных аудио в VK", "info");
                    }
                  }}
                  style={{
                    padding: 24, borderRadius: 20,
                ...
                >
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.08)", padding: "4px 10px", borderRadius: 12, fontSize: 11, fontWeight: 700, color: "#fff", marginBottom: 12 }}>
                    <ListMusic size={12} /> ВКОНТАКТЕ
                  </div>
                  <h3 style={{ fontSize: 22, fontWeight: 800, color: "#fff", marginBottom: 6 }}>Мои Аудиозаписи</h3>
                  <p style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginBottom: 18 }}>{vkUserTracks.length} сохраненных треков в вашем профиле VK</p>
                  <button style={{ background: "rgba(255,255,255,0.12)", color: "#fff", border: "none", padding: "10px 20px", borderRadius: 20, fontWeight: 700, fontSize: 13, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>
                    <Play size={14} fill="currentColor" /> Включить мои аудио
                  </button>
                </div>
              </div>

              {/* Large Release Cover Cards Grid */}
              {getFilteredVkRecs().length > 0 && (
                <div style={{ marginBottom: 36 }}>
                  <div className="vk-section-header">
                    <div className="vk-section-title">
                      Микс:
                    </div>
                    <div className="vk-section-subtitle">{getFilteredVkRecs().length} треков</div>
                  </div>
                  <div className="vk-releases-grid">
                    {getFilteredVkRecs().map((t, i) => (
                      <div 
                        key={`vkcard-${t.id}-${i}`}
                        className="vk-release-card"
                        onClick={() => playTrack(t, i, false, getFilteredVkRecs())}
                      >
                        <div className="vk-release-card__cover-wrapper">
                          <img 
                            src={t.thumbnail || "/vk_logo.png"} 
                            alt={t.title} 
                            className="vk-release-card__img" 
                            onError={(e) => { e.target.onerror = null; e.target.src = "/vk_logo.png"; }}
                          />
                          <div className="vk-release-card__play-btn">
                            <Play size={22} fill="currentColor" style={{ marginLeft: 2 }} />
                          </div>
                          <div className="vk-release-card__badge">
                            <VkLogo size={12} />
                          </div>
                        </div>
                        <div className="vk-release-card__title" title={t.title}>{t.title}</div>
              ...
              {/* VK User Saved Tracks List */}
              {vkUserTracks.length > 0 && (
                <div className="home-section">
                  <div className="home-section__header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 18, fontWeight: 700 }}>Мои Аудиозаписи VK</span>
                    <span style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>{vkUserTracks.length} треков</span>
                  </div>
                  <div className="track-list">
                    {vkUserTracks.map((t, i) => 
                      renderTrackRow(t, i, { key: `vkuser-${t.id}-${i}`, playTrack: (trk) => { playTrack(trk, i, false, vkUserTracks); } })
                    )}
                  </div>
                </div>
              )}
...
{activeView==="settings"&&(
              ...
              <div className="settings-container">
                <div className="settings-sidebar">
                  <div className="settings-sec-group">
                    ...
                    <div className={`settings-sidebar-item ${settingsSubTab === "vk" ? "active" : ""}`} onClick={() => setSettingsSubTab("vk")}>
                      <img src="/vk_logo.png" alt="VK" style={{ width: 14, height: 14, objectFit: "contain" }}/> <span>VK Музыка</span>
                    </div>
                ...
                <div className="settings-content">
                  {settingsSubTab === "vk" && (
                    <div className="settings-card">
                      <div className="settings-card__title" style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <img src="/vk_music_logo.png" alt="VK Музыка" style={{ height: 22, objectFit: "contain" }} />
                      </div>
                      <p className="settings-card__desc">Слушайте музыку ВКонтакте напрямую. Поиск и стриминг работают автоматически без регистрации! Для поиска по своим личным аудиозаписям можно привязать токен VK.</p>
                      <div className="settings-yandex-row" style={{ display: "flex", gap: 10, marginTop: 12 }}>
                        <input 
                          className="settings-input" 
                          style={{ flex: 1 }} 
                          type="password" 
                          placeholder="Вставьте токен или полную ссылку из браузера..." 
                          value={vkToken} 
                          onChange={e => handleVkTokenInput(e.target.value)} 
                          onClick={e=>e.stopPropagation()}
                        />
                        <button 
                          type="button" 
                          className="save-btn" 
                          style={{ whiteSpace: "nowrap", background: "#0077ff", color: "#fff", fontWeight: "600", padding: "0 18px", borderRadius: 10 }}
                          onClick={() => invoke("start_vk_oauth").catch(e => setErrorMessage(e.toString()))}
                        >
                          Вход в «Маруся»
                        </button>
                      </div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginTop: 6 }}>
                        Нажмите «Вход в «Маруся»» для автоматической авторизации, либо откройте <a href="https://oauth.vk.com/authorize?client_id=6463690&scope=audio,offline&response_type=token" target="_blank" rel="noreferrer" style={{ color: "#0077ff", fontWeight: "bold" }}>ссылку авторизации «Маруся» (Client ID 6463690 с VKHost)</a> и вставьте итоговый адрес.
                      </div>
                      {vkToken && <div className="settings-status settings-status--ok" style={{ marginTop: 12 }}>✓ Токен VK подключен</div>}

                      <div style={{ marginTop: 24, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div>
                          <div style={{ fontWeight: 600, fontSize: 14 }}>Отображать VK Музыку на боковой панели</div>
                          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>Добавляет кнопку со страницей рекомендаций VK на панель под "Главная"</div>
                        </div>
                        <button 
                          className="save-btn" 
                          style={{ background: showVkHome ? "rgba(0, 119, 255, 0.2)" : "rgba(255,255,255,0.08)", border: showVkHome ? "1px solid #0077ff" : "1px solid rgba(255,255,255,0.15)", color: "#fff", padding: "6px 14px", borderRadius: 8, cursor: "pointer" }}
                          onClick={() => {
                            setShowVkHome(prev => {
                              const next = !prev;
                              localStorage.setItem("show_vk_home", next ? "true" : "false");
                              return next;
                            });
                          }}
                        >
                          {showVkHome ? "✓ Включено" : "Выключено"}
                        </button>
                      </div>
                    </div>
                  )}
      ...
      <div className="toast-container">
        {toasts.map(t => {
          const isLike = t.type === "like" || t.message.includes("❤️") || t.message.includes("лайк");
          const isDislike = t.type === "dislike" || t.message.includes("💔") || t.message.includes("Убрано");
          const isMusic = t.message.includes("🎵") || t.message.includes("кэш");

          return (
            <div key={t.id} className={`toast toast--${t.type}`}>
              <span className="toast__icon" style={{ display: "flex", alignItems: "center" }}>
                {isLike ? (
                  <img src="https://em-content.zobj.net/source/apple/354/red-heart_2764-fe0f.png" alt="❤️" className="ios-emoji-img" onError={e=>{e.target.style.display="none";}} />
                ) : isDislike ? (
                  <img src="https://em-content.zobj.net/source/apple/354/broken-heart_1f494.png" alt="💔" className="ios-emoji-img" onError={e=>{e.target.style.display="none";}} />
                ) : isMusic ? (
                  <img src="https://em-content.zobj.net/source/apple/354/musical-notes_1f3b6.png" alt="🎵" className="ios-emoji-img" onError={e=>{e.target.style.display="none";}} />
                ) : t.type === "success" ? (
                  <img src="https://em-content.zobj.net/source/apple/354/check-mark-button_2705.png" alt="✅" className="ios-emoji-img" onError={e=>{e.target.style.display="none";}} />
                ) : (
                  <img src="https://em-content.zobj.net/source/apple/354/sparkles_2728.png" alt="✨" className="ios-emoji-img" onError={e=>{e.target.style.display="none";}} />
                )}
              </span>
              <span className="toast__text">{t.message.replace(/^[❤️💔🎵✅⚠️✨✕]\s*/, "")}</span>
            </div>
          );
        })}
      </div>
